...
export const PostsPreview = ({ onPostsPage = false }: PostsPreviewProps) => {
  ...
  return (
    <>
      <article className="bg-gray w-full flex-1 shadow-md shadow-lightGray flex flex-col justify-around">
        {posts.map(({ metadata, content }, i) => {
          return (
            <Link
              href={`/posts/${metadata.slug}`}
              key={i}
              className="grid grid-cols-5 p-[25px]"
            >
              <div className="col-span-2 bg-black mr-6" />
